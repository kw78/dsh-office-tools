...
- CI's node 20 job (broken since 0.6.0: the workflow pinned pnpm 11.7.0, whose setup path fails under node 20, while `pnpm-lock.yaml` is actually generated by pnpm 10): `packageManager` is now pinned to `pnpm@10.34.5` — the lockfile generator — and `pnpm/action-setup` reads it instead of a hard-coded version, making the toolchain single-sourced and node-20-green again.
